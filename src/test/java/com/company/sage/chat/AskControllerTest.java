...
class AskControllerTest {
    ...
    void setUp() {
        ...
        SemanticSearchTool semanticTool = new SemanticSearchTool(graphRagClient, retrievalProperties);
        GraphTraversalTool graphTool = new GraphTraversalTool(graphRagClient, retrievalProperties);
        ResultMergerTool mergerTool = new ResultMergerTool(scoringProperties, retrievalProperties);

        // Stub LLM model to return expected event sequences
        ...
        sageRootAgent = sageAgents.sageRootAgent(queryInterpret, parallelRetrieve, merger, synth);

        AskController askController = new AskController(sageRootAgent);
        mockMvc = MockMvcBuilders.standaloneSetup(askController).build();
    }

    @Test
    void testAskEndpointValidationFailure() throws Exception {
        AskRequest badRequest = new AskRequest("");
    ...
    @Test
    void testAskEndpointSseSuccess() throws Exception {
        ...
        System.out.println("SSE Stream output:\n" + sseOutput);
